...
#The following code is broken into several chunks. The first major chunk, takes SliM output files 
#and converts them to a dataframe to obe used as input for STIMS. The second part of code, creates 
#a gene id file that divides each of the genomic elements in the SLiMulation into 1000 genes,
#each of size 100. This file only needed to be created once. Therefore, lines of code in this 
#chunk are commented out. The last chunk of code, takes SLiM output data and creates mutation 
#trajectories for each. Each section is labeled chunk 1 ... X. 
...
#I only needed to create the genome file once, so here it is commented out. 
#This chunk of code has been left in my script for reproducibility. 
# n <- 400
# prefix <- "g"
# suffix <- seq(1:n)
...
# }
# 
# matrix.data <- data.frame(Gene, start, end, gene_length)
# write.csv(matrix.data, "SLiM_geneIDs.csv", quote = F, row.names =F)
